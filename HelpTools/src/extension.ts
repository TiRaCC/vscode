...
export function activate(context: vscode.ExtensionContext) {
	...
	let disposable = vscode.commands.registerCommand('HelpTools.CreateNewClass', (url) => {
		let author: string | undefined = context.workspaceState.get("author");
...
async function createNewSkinClass(url: vscode.Uri, skinName: string, classUrl: string, className: any): Promise<void> {
	...
	skinItem = await traceChild(skinItem, idNodes);
	let date = new Date();
	let dateStr = date.getFullYear() + "-" + (date.getMonth() + 1) + "-" + date.getDate() + " " + date.getHours() + ":" + date.getMinutes() + ":" + date.getSeconds();
...
//解析子集
function analysisChild(data: any, isItem: boolean = false): IChildNodes[] {
	...
	let findChildFun = function (parent: any, type: string) {
		...
		for (var tempProperty in parent) {
			if (
				...
				tempProperty === "e:itemRendererSkinName" ||
			...
			if (tempChild.id !== null && tempChild.id !== undefined) {
				...
				} else if (!isItem && type === "solar:Component" && tempChild.skinName) {
					data.skin = tempChild.skinName;
				}
				idNodes.push(data);
			}
			if (tempProperty.indexOf('e:') === 0 || tempProperty.indexOf("solar:") === 0 || tempProperty.indexOf("ns1:") === 0) {
...
async function analysisTarget(url: vscode.Uri, isItem: boolean = false) {
	let data = await vscode.workspace.fs.readFile(url);
	let json = await parseString(Uint8ArrayToString(data));
	return analysisChild(json, isItem);
}
...
async function traceChild(skinItem: ISkinItem, idNodes: IChildNodes[], isItem: boolean = false): Promise<ISkinItem> {
	...
	for (let i: number = 0, len: number = idNodes.length; i < len; i++) {
		...
		let eType = "";
		if (element.type === "solar:Component") {
...
function rootUrl(): string {
	...
	if (rootUrl?.indexOf("slagman/slagman") === -1) {
		rootUrl += "/slagman";
	};
	return rootUrl || "";
}
